...
export const transactionhistory = async (req, res) => {
  try {
    ...
    // Fetch all ledger entries and populate transfer details
    const ledgerEntries = await Ledger.find({ userEmail: email })
      .populate("transferId")
      .sort({ createdAt: -1 });

    const history = ledgerEntries.map((entry) => {
      const transfer = entry.transferId;
      ...
      return {
        ...
        type: entry.type,
        status: entry.status,
        counterparty: isDebit ? transfer.receiverEmail : transfer.senderEmail,
